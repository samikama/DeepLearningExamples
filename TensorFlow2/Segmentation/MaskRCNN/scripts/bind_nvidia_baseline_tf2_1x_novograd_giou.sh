...
/opt/amazon/openmpi/bin/mpirun  --tag-output --mca plm_rsh_no_tree_spawn 1 \
    ...
    --oversubscribe \
    --bind-to none \
    /shared/sami/conda/bin/python ${BASEDIR}/bind_launch.py  --direct_launch=${DIRECT_LAUNCH} --nproc_per_node=8 --nsockets_per_node=2 --ncores_per_socket=24 ${BASEDIR}/../mask_rcnn_main.py \
        --mode="train_and_eval" \
	--loop_mode="tape" \
	--box_loss_type="giou" \
