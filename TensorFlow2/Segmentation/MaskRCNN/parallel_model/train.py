...
tf.config.set_visible_devices(
    [physical_devices[0 * 2], physical_devices[0 * 2 + 1]], 'GPU')
...
train_tdf_0 = train_tdf.map(map0,
                            num_parallel_calls=tf.data.experimental.AUTOTUNE)
train_tdf_1 = train_tdf.map(map1,
                            num_parallel_calls=tf.data.experimental.AUTOTUNE)
train_tdf_0 = train_tdf_0.apply(
    tf.data.experimental.prefetch_to_device(devices[0].name))
train_tdf_1 = train_tdf_1.apply(
    tf.data.experimental.prefetch_to_device(devices[1].name))
...
train_op_0, train_op_1, total_loss_0, total_loss_1 = model.model(
    features_0, features_1, params, devices, labels_0, labels_1)
...
_init_op_0, _init_feed_dict_0 = load_weights.assign_from_checkpoint(
    checkpoint_file, var_list_0)
_init_op_1, _init_feed_dict_1 = load_weights.assign_from_checkpoint(
    checkpoint_file, var_list_1)
...
def do_step_profile(profile_path, sess, stepstr, progressbar, fetch_ops):
  if do_profile:
    ...
    p_bar.set_description("Loss 0: {0:.4f}, Loss 1: {1:.4f}".format(
        ma_loss_0, ma_loss_1))
    tf_profiler.stop()
  else:
    for i in progressbar:
...
with tf.Session() as sess:
  ...
  timings = []
  for i in p_bar:
    _, _, loss_0, loss_1 = sess.run(
        [train_op_0, train_op_1, total_loss_0, total_loss_1])
  loss_history_0 = []
  loss_history_1 = []
  p_bar = tqdm(range(200))
  if do_profile:
    do_step_profile(profile_path, sess, stepstr, p_bar,
                    [train_op_0, train_op_1, total_loss_0, total_loss_1])
  else:
    for i in p_bar:
