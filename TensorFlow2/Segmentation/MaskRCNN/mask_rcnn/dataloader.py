...
if __name__ == "__main__":
  ...
  import numpy as np
  #np.set_printoptions(threshold=np.inf,precision=4,linewidth=900,floatmode='fixed')
  ...
  parser.add_argument("--validate_preprocessed",
                      default=False,
                      action="store_true",
                      help="Use preprocessed data")
  ...
  ds_params = {
      ...
      "disable_options": False,
      "validate_preprocessed":FLAGS.validate_preprocessed
  }
  ...
  if not FLAGS.tf2:
    ...
    for step in range(TOTAL_STEPS):
      ...
      if FLAGS.validate_preprocessed:
        n_masks=labels["num_masks"].numpy()
        orig=labels["cropped_orig_gt_masks"].numpy()[0,:n_masks[0]]
        orig=np.where(orig>=0.5,1.,0.)
        cached=labels["cropped_gt_masks"].numpy()[0,:n_masks[0]]
        cached=np.where(cached>=0.5,1.,0.)
        if not np.allclose(orig,cached):
          logging.error(f"mask values differ at step {step}!")
          print(np.sum(np.abs(orig-cached)>0.001),"orig mean=",np.mean(orig),np.std(orig),"cached",np.mean(cached),np.std(cached))
          assert(False)
