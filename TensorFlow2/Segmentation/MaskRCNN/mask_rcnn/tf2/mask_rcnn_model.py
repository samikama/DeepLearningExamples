  ...
  if params["amp"]:
    ...
    if params['loss_scale'] > 0:
      optimizer = tf.compat.v1.train.experimental.MixedPrecisionLossScaleOptimizer(
          optimizer, loss_scale=params['loss_scale'])
    else:
      optimizer = tf.compat.v1.train.experimental.MixedPrecisionLossScaleOptimizer(
...
def create_lamb_optimizer(learning_rate, params):
  ...
  if params["amp"]:
    loss_scale = tf.train.experimental.DynamicLossScale(
        initial_loss_scale=(2**11), increment_period=2000, multiplier=2.0)
    optimizer = tf.compat.v1.train.experimental.MixedPrecisionLossScaleOptimizer(
        optimizer, loss_scale=loss_scale)
...
class MRCNN(tf.keras.Model):
  ...
  def call(self, features, labels, params, is_training=True):
    ...
    if not is_training:
      ...
      model_outputs.update({
          ...
          'class_targets':
              class_targets,
          'box_targets':
              box_targets if is_iou_based_loss(params['box_loss_type']) else
              encoded_box_targets,
          'box_rois':
              rpn_box_rois,
      })
    ...
    mask_outputs = self.mask_head(inputs=mask_roi_features,
...
def _model_fn(features, labels, mode, params):
  ...
  model = MRCNN(params)
  model_outputs = model(features, labels, params,
  ...
  if mode == tf.estimator.ModeKeys.TRAIN:
    ...
    if global_gradient_clip_ratio > 0.0:
      ...
      (clipped_grads, _) = tf.clip_by_global_norm(
          gradients,
          clip_norm=global_gradient_clip_ratio,
    ...
    for grad, var in zip(gradients, variables):
      ...
      if grad is not None and any(
          [pattern in var.name for pattern in ["bias", "beta"]]):
        grad = 2.0 * grad
  ...
  if not isinstance(replica_id,
                    tf.Tensor) or tf.get_static_value(replica_id) == 0:

    register_metric(name="L2 loss",
                    tensor=l2_regularization_loss,
                    aggregator=StandardMeter())
    register_metric(name="Mask loss",
                    tensor=mask_loss,
                    aggregator=StandardMeter())
    register_metric(name="Total loss",
                    tensor=total_loss,
                    aggregator=StandardMeter())

    register_metric(name="RPN box loss",
                    tensor=rpn_box_loss,
                    aggregator=StandardMeter())
    register_metric(name="RPN score loss",
                    tensor=rpn_score_loss,
                    aggregator=StandardMeter())
    register_metric(name="RPN total loss",
                    tensor=total_rpn_loss,
                    aggregator=StandardMeter())

    register_metric(name="FastRCNN class loss",
                    tensor=fast_rcnn_class_loss,
                    aggregator=StandardMeter())
    register_metric(name="FastRCNN box loss",
                    tensor=fast_rcnn_box_loss,
                    aggregator=StandardMeter())
    register_metric(name="FastRCNN total loss",
                    tensor=total_fast_rcnn_loss,
                    aggregator=StandardMeter())

    register_metric(name="Learning rate",
...
class SessionModel(object):
  ...
  def __init__(self,
    ...
    train_params = dict(run_config.values(),
    ...
    eval_params = dict(run_config.values(),
  ...
  def train_fn(self):
    ...
    model_outputs = self.forward(features, labels, self.run_config.values(),
                                 True)
    model_outputs.update({
        'source_id': features['source_ids'],
        'image_info': features['image_info'],
    ...
    if self.run_config.optimizer_type in ['LAMB',
                                          'Novograd']:  # decoupled weight decay
      l2_regularization_loss = tf.constant(0.0)
    else:
    ...
    if self.run_config.optimizer_type == 'SGD':
      ...
      optimizer = create_novograd_optimizer(learning_rate,
                                            self.run_config.values())
    else:
      raise NotImplementedError
  ...
  @staticmethod
  def get_session_config(use_xla=True,
                         use_amp=True,
...
class TapeModel(object):
  ...
  def __init__(self,
    ...
    train_params = dict(self.params.values(),
                        batch_size=self.params.train_batch_size)
    self.train_tdf = iter(train_input_fn(train_params)) \
                        if train_input_fn else None
    eval_params = dict(self.params.values(),
  ...
  def get_optimizer(self):
    if self.params.lr_schedule == 'piecewise':
      ...
      schedule = tf.keras.experimental.CosineDecay(
          self.params.init_learning_rate, self.params.total_steps, alpha=0.001)
    else:
      raise NotImplementedError
    schedule = warmup_scheduler.WarmupScheduler(
        schedule, self.params.warmup_learning_rate, self.params.warmup_steps)
  ...
  def train_step(self,
    ...
    with tf.GradientTape() as tape:
      ...
      if self.params.optimizer_type in ['LAMB',
                                        'Novograd']:  # decoupled weight decay
        loss_dict['l2_regularization_loss'] = tf.constant(0.0)
      else:
        loss_dict[
            'l2_regularization_loss'] = self.params.l2_weight_decay * tf.add_n([
                tf.nn.l2_loss(v)
    ...
    if is_herring():
      ...
      if self.params.amp:
        ...
        gradients = tape.gradient(loss_dict['total_loss'],
                                  self.forward.trainable_variables)
      ...
      if MPI_is_distributed(False):
        tape = hvd.DistributedGradientTape(
            tape, compression=hvd.compression.NoneCompressor)
      if self.params.amp:
        scaled_gradients = tape.gradient(scaled_loss,
                                         self.forward.trainable_variables)
        gradients = self.optimizer.get_unscaled_gradients(scaled_gradients)
  ...
  def initialize_model(self):
    ...
    tf.get_logger().info(
        f"{gen_log_prefix()} img_id= {features['source_ids'].numpy()[0]} (init_model) synch_weights={True}, sync_opt={True}"
    )
    model_outputs = self.train_step(features, labels,
                                    tf.convert_to_tensor(True),
                                    tf.convert_to_tensor(True), True, True)
    features, labels = next(self.train_tdf)
    tf.get_logger().info(
        f"{gen_log_prefix()} img_id= {features['source_ids'].numpy()[0]} (init_model) synch_weights={True}, sync_opt={False}"
    )
    model_outputs = self.train_step(features, labels,
                                    tf.convert_to_tensor(True),
                                    tf.convert_to_tensor(False), True, True)
    features, labels = next(self.train_tdf)
    tf.get_logger().info(
        f"{gen_log_prefix()} img_id= {features['source_ids'].numpy()[0]} (init_model) synch_weights={False}, sync_opt={False}"
    )
    model_outputs = self.train_step(features, labels,
  ...
  def train_epoch(self, steps, broadcast=False, profile=None):
    ...
    if MPI_rank(is_herring()) == 0 and profile is not None:
      ...
      for i in p_bar:
        ...
        if MPI_rank() == 0:
          ...
          p_bar.set_description("Loss: {0:.4f}, LR: {1:.4f}".format(
              mean(loss_history[-50:]), learning_rate))
      tf_profiler.stop()
    else:
      runtype = "TrainStep"
      self.st_global = time.time()
      for i in p_bar:
        if i == 2 and self.epoch_num == 0:
          self.st = time.time()
        if broadcast and i == 0:
          b_w, b_o = True, True
        elif i == 0:
    ...
    logging.info(
        f"Rank={MPI_rank()} Avg step time all  {np.mean(times)*1000.} +/- {np.std(times)*1000.} ms"
    )
    logging.info(
        f"Rank={MPI_rank()} Avg step time [1:] {np.mean(times[1:])*1000.} +/- {np.std(times[1:])*1000.} ms"
    )
    logging.info(
        f"Rank={MPI_rank()} Avg step time [2:] {np.mean(times[2:])*1000.} +/- {np.std(times[2:])*1000.} ms"
    )
    logging.info(
        f"Rank={MPI_rank()} Avg step time [3:] {np.mean(times[3:])*1000.} +/- {np.std(times[3:])*1000.} ms"
    )
    if MPI_rank(is_herring()) == 0:
      if self.epoch_num == 16:
        print(f'Total time is {time.time() - self.st_global}')
  ...
  def get_latest_checkpoint(self):
    try:
      return sorted(
          [_ for _ in os.listdir(self.model_dir) if _.endswith(".h5")])[-1]
    except:
      return None
  ...
  def predict(self, features):
    labels = None
    model_outputs = self.forward(features,
                                 labels,
                                 self.params.values(),
                                 is_training=False)
    model_outputs.update({
  ...
  def run_eval(self,
    ...
    stop_event = mp.Event()
    stop_event.clear()
    post_proc = mp.Process(target=coco_pre_process,
                           args=(in_q, out_q, stop_event))
    post_proc2 = mp.Process(target=coco_pre_process,
                            args=(in_q, out_q, stop_event))
    post_proc3 = mp.Process(target=coco_pre_process,
                            args=(in_q, out_q, stop_event))
    post_proc.start()
    post_proc2.start()
    post_proc3.start()
    ...
    if (not use_dist_coco_eval):
      ...
      predictions_list = evaluation.gather_result_from_all_processes(
        ...
        if use_ext:
          ...
          if async_eval:
            eval_thread = threading.Thread(
                target=evaluation.compute_coco_eval_metric_n,
                name="eval-thread",
                args=args)
            eval_thread.start()
          else:
      ...
      end_gather_result = time.time()
      validation_json_file = self.params.val_json_file
      bbox_score, mask_score = evaluation.fast_eval(converted_predictions,
                                                    validation_json_file,
                                                    use_ext, use_dist_coco_eval)
    ...
    if (MPI_rank(is_herring()) == 0):
      ...
      print(
          f"(avg, total) DataLoad ({data_load_total/steps}, {data_load_total}) predict ({predict_total/steps}, {predict_total})"
      )
      print(
          f"Total Time {end_coco_eval-start_total_infer} Total Infer {end_total_infer - start_total_infer} gather res {end_gather_result - end_total_infer} coco_eval {end_coco_eval - end_gather_result}"
      )
...
def coco_pre_process(in_q, out_q, finish_input):
  ...
  while (not finish_input.is_set() or in_q.qsize() > 0):
    try:
      ...
      converted_predictions += coco.load_predictions(worker_predictions,
                                                     include_mask=True,
                                                     is_image_mask=False)
