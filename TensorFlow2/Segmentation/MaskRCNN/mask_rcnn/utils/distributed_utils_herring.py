...
def MPI_is_distributed(run_herring=True):
...
def MPI_local_rank(run_herring=True):
    return herring.local_rank()


def MPI_rank(run_herring=True):
    return herring.rank()


def MPI_size(run_herring=True):
    return herring.size()


def MPI_rank_and_size(run_herring=True):
